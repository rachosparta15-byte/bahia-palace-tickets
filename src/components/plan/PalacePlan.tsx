 ...
 *
 * WHY THE DRAWING LOOKS LIKE THIS
 * The first version drew every space as a filled rectangle and joined their
 * centres with straight lines. Architecturally that was backwards: a Moroccan
 * palace is organised around open courtyards — the riad IS the garden and the
 * building is the frame around it — so drawing the courts as solid mass
 * inverted the one thing a plan of this building exists to show, and what was
 * left read as a flowchart. Courts are now voids inside a gallery, the garden
 * carries its four-part division, rooms are hatched mass, and the apartments
 * read as the row of small cells they are. The route turns corners through
 * doorways instead of flying through walls. The eight-pointed khatam marking
 * each fountain is the motif in the zellige underfoot.
 *
 * WHY EVERY STRING IS A PROP
 * This is a client component. Reading the message catalogue here would ship
 * the whole catalogue to the browser for the sake of a dozen labels. The
 * server page translates once and passes the result down, so the bundle
 * carries no copy at all and the diagram works in seven languages without
 * knowing any of them.
 *
...
import {
  ROUTE, SPUR, STOPS, VIEWBOX, type SpaceCopy, type PlanChrome,
...
/** Evenly spaced column marks along the gallery. */
...
function SpaceShape({ s, on }: { s: SpaceCopy; on: boolean }) {
  const { x, y, w, h } = s.box;
...
export function PalacePlan({ spaces, chrome }: { spaces: SpaceCopy[]; chrome: PlanChrome }) {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)] gap-8 items-start">
      ...
      <div>
        <svg
          ...
          role="group"
          aria-label={chrome.diagramLabel}
        >
          <defs>
          ...
          {/* The palace is a walled compound before it is anything else. */}
          ...
          <text x={200} y={640} textAnchor="middle" fontSize={12} fill="#8C7355">{chrome.street}</text>
        </svg>
        ...
        <div className="flex flex-wrap gap-x-5 gap-y-1.5 mt-3 text-[11px] text-[#8C7355]">
          ...
          <span>{chrome.legendCourt}</span>
        </div>

        <p className="text-xs text-[#8C7355] mt-3 leading-relaxed">{chrome.schematicNote}</p>
      </div>
      ...
      <div className="lg:sticky lg:top-6">
        <div className="bg-[#251A0F] border border-[rgba(232,163,61,0.15)] rounded-2xl p-6">
          ...
          <h4 className="text-[#F5E8CC] font-semibold text-sm mb-2">{chrome.noticeTitle}</h4>
          ...
          {active.story && (
            <div className="border-t border-[rgba(232,163,61,0.15)] pt-4">
              <h4 className="text-[#F5E8CC] font-semibold text-sm mb-2">{chrome.storyTitle}</h4>
              <p className="text-sm text-[#C4A882] leading-relaxed">{active.story}</p>
              {active.source && <p className="text-[11px] text-[#8C7355] mt-2.5">{active.source}</p>}
            </div>
          )}
        </div>
        ...
        <p className="text-xs text-[#8C7355] mt-4 px-3">{chrome.totalTime}</p>
      </div>
    </div>
  );
}
