...
export function OrnamentDivider({ label }: { label?: string }) {
  return (
    <div className="flex items-center gap-4 text-[#E8A33D] my-6">
      <div className="flex-1 h-px bg-gradient-to-r from-transparent via-[#D4BC96] to-transparent" />
      <ZelligePattern className="w-8 h-8 text-[#E8A33D]" />
      {label && <span className="text-sm font-medium text-[#5C3D20] whitespace-nowrap">{label}</span>}
      <ZelligePattern className="w-8 h-8 text-[#E8A33D]" />
      <div className="flex-1 h-px bg-gradient-to-r from-transparent via-[#D4BC96] to-transparent" />
    </div>
  );
